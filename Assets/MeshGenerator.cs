...
public class MeshGenerator : MonoBehaviour
{
    ...
    // Takes in vertex for key and list of triangles for value.
    ...
    {
        // Triangulate square based on configuration.
    ...
    void MeshFromPoints(params Node[] points)
    {
        ...
        // If we have 3 or more points, creta a triangle.
        if (points.Length >= 3)
        {
            CreateTriangle(points[0], points[1], points[2]);
        }
        // If we have 4 or more points, create a quad.
        if (points.Length >= 4)
        {
            CreateTriangle(points[0], points[2], points[3]);
        }
        // If we have 5 points, create a pentagon.
        if (points.Length >= 5)
        {
            CreateTriangle(points[0], points[3], points[4]);
        }
        // If we have 6 points, create a hexagon.
        if (points.Length >= 6)
        {
    ...
    void CalculateMeshOutlines()
    {
        ...
        {
            // Only check vertices we haven't already checked. For optimization.
            if (!checkedVertices.Contains(vertexIndex))
            {
                int newOutlineVertex = GetConnectedOutlineVertex(vertexIndex);
                if (newOutlineVertex != -1)
                {
                    // It's not connected, so add it as checked.
